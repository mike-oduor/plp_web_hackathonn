...
function displayFullName(fullName) {
    ...
    const selectCourse = (courseId) => {
        const userId = getUserId(); // You need to implement a function to get the logged-in user's ID
        fetch('/select-course', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
    ...
    const displaySelectedCourses = (userId) => {
        fetch(`/user-courses/${userId}`)
        .then(response => response.json())
        .then(selectedCourses => {
            const selectedCoursesList = document.getElementById('selected-courses');
            selectedCoursesList.innerHTML = '<h2>Selected Courses</h2>';
            selectedCourses.forEach(selectedCourse => {
                const courseElement = document.createElement('div');
                courseElement.textContent = selectedCourse.name;
